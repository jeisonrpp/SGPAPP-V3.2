...
namespace SGPAPP
{
    public partial class frmCitasd : Form
    {
        ...
        private void frmCitasd_Load(object sender, EventArgs e)
        {
        ...
        public void GetData()
        {
            try
            {
                using (var con = new SqlConnection(conect))
                {
                    ...
                    con.Open();
                    cmd = new SqlCommand("select CONVERT(varchar(15),CAST(chora AS TIME),100) as [Hora de Cita], RTRIM(pNom) as [Paciente], RTRIM(pComment) as [Comentario], RTRIM(cEstatus) as [Estado de Cita], RTRIM(cId) as [Id de Cita] , RTRIM(cFecha) as [Fecha] from tbCitas where cFecha = '" + cambiada1 + "' and cTipo = 'Domicilio' order by cHora", con);
        ...
        private void frmCitasd_DoubleClick(object sender, EventArgs e)
        {
